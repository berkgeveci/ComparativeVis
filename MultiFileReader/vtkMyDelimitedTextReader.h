/*=========================================================================
  ...
  Module:    $RCSfile: vtkMyDelimitedTextReader.h,v $
...
-------------------------------------------------------------------------*/
// .NAME vtkMyDelimitedTextReader - reader for pulling in flat text files
//
// .SECTION Description
// vtkMyDelimitedTextReader is an interface for pulling in data from a
...
#ifndef __vtkMyDelimitedTextReader_h
#define __vtkMyDelimitedTextReader_h
...
class VTK_EXPORT vtkMyDelimitedTextReader : public vtkTableAlgorithm
{
public:
  static vtkMyDelimitedTextReader* New();
  vtkTypeRevisionMacro(vtkMyDelimitedTextReader,vtkTableAlgorithm);
  ...
  // Description:
  // If InputString is set, the reader will read from it
  // instead of the file. Note: if InputString, the FileName
  // will be ignored even if it is set.
  vtkSetStringMacro(InputString);
  vtkGetStringMacro(InputString);
  
 protected:
  vtkMyDelimitedTextReader();
  ~vtkMyDelimitedTextReader();

  vtkMyDelimitedTextReaderInternals* Internals;
...
private:
  vtkMyDelimitedTextReader(const vtkMyDelimitedTextReader&); // Not implemented
  void operator=(const vtkMyDelimitedTextReader&);   // Not implemented
};
